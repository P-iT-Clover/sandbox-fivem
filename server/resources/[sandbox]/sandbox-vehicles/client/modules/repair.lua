...
exports("RepairFull", function(veh)
	local nSuccess = exports['sandbox-vehicles']:RepairNormal(veh)
	local pSuccess = exports['sandbox-vehicles']:RepairPart(veh, false)
		...
		if LocalPlayer.state.loggedIn then
			...
			if vehicle and DoesEntityExist(vehicle) and CanRepairVehicle(vehicle) then
				if exports['sandbox-vehicles']:RepairNeedsKit(vehicle, type) then
					...
					Wait(500)
					exports['sandbox-vehicles']:SyncDoorsOpen(vehicle, 4, false, false)
					exports['sandbox-hud']:Progress({
						...
						if not cancelled and CanRepairVehicle(vehicle) then
							local success, errorMessage = exports['sandbox-vehicles']:RepairKit(vehicle, type)
							cb(success)

							exports['sandbox-vehicles']:SyncDoorsShut(vehicle, 4, false)

							if not success then
								if errorMessage then
									exports["sandbox-hud"]:Notification("error", errorMessage)
								else
									exports["sandbox-hud"]:Notification("error", "Repair Failed")
								end
							else
								exports["sandbox-hud"]:Notification("success", "Repaired Successfully")
							end
						else
							cb(false)
						end
					...
					exports["sandbox-hud"]:Notification("error", "Vehicle Doesn't Need Repair")
					cb(false)
				end
			else
				cb(false)
...
RegisterNetEvent("VehicleSync:Client:BodyRepair", function(vNet)
	if NetworkDoesEntityExistWithNetworkId(vNet) then
		...
		if DoesEntityExist(vehicle) then
			exports['sandbox-vehicles']:RepairNormal(vehicle, true)
		end
	end
end)
...
RegisterNetEvent("Vehicles:Client:Repair:Normal", function(netId, ...)
	...
	exports['sandbox-vehicles']:RepairNormal(entity, ...)
end)

RegisterNetEvent("Vehicles:Client:Repair:Full", function(netId, ...)
	local entity = NetworkGetEntityFromNetworkId(netId)

	exports['sandbox-vehicles']:RepairFull(entity, ...)
end)

RegisterNetEvent("Vehicles:Client:Repair:Engine", function(netId, ...)
	local entity = NetworkGetEntityFromNetworkId(netId)

	exports['sandbox-vehicles']:RepairEngine(entity, ...)
end)
